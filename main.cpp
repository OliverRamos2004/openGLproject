...
int main() {
    ...
    Mesh chair(std::string(ASSETS_DIR) + "chair_01.obj", cubeShader.getID());

    GLuint cube_diffuse = cubeShader.bindTexture2D("material.diffuse", std::string(ASSETS_DIR) + "container2.png", 0,
                                                   false);
    GLuint cube_specular = cubeShader.bindTexture2D("material.specular",
                                                    std::string(ASSETS_DIR) + "container2_specular.png", 1, false);
    ...
    cfg.dirLights.clear();          // no sun light
    cfg.spotLights.clear();         // we’ll add spotlight later
    cfg.pointLights.resize(4);      // ceiling strip lights

    float ceilingY = 6.0f;
    float spacing = 4.0f;

    for (int i = 0; i < cfg.pointLights.size(); i++) {
        cfg.pointLights[i].position =
            glm::vec3(-6.0f + i * spacing, ceilingY, 0.0f);

        cfg.pointLights[i].ambient  = glm::vec3(0.02f);
        cfg.pointLights[i].diffuse  = glm::vec3(0.35f);
        cfg.pointLights[i].specular = glm::vec3(0.6f);

        cfg.pointLights[i].linear    = 0.14f;
        cfg.pointLights[i].quadratic = 0.07f;
    }



    cfg.spotLights[0].position = glm::vec3(0.0f, 6.5f, 0.0f); // ceiling center
    cfg.spotLights[0].direction = glm::vec3(0.0f, -1.0f, 0.0f); // pointing down
    ...
    cfg.spotLights[0].diffuse = glm::vec3(0.8f, 0.75f, 0.7f); // warm
    cfg.spotLights[0].specular = glm::vec3(1.0f);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        cubeShader.setUniform("exposure", 0.8f); // temple-dim mood
        ...
        for (int i = 0; i < chairCount; i++) {
            float theta = (float)i / chairCount * glm::two_pi<float>();
            ...
            // rotate chair to face the center
            float yaw = atan2(-x, -z);
            model = glm::rotate(model, yaw, glm::vec3(0, 1, 0));

            // scale if needed (many OBJ chairs are huge)
